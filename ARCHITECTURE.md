...
The auth dialog reuses existing visual language instead of a dialog-specific one: the primary action is a filled `--editing` pill with `--editing-foreground` text like the canvas `Button` node, the secondary action is the toolbar cancel button's outlined treatment, and the headings use the `display-5` type scale. Because the dialog is shorter than the drawer's minimum height, the drawer centers it vertically rather than pinning it to the top. That way the first thing a visitor sees already looks like the editor they are about to enter.
