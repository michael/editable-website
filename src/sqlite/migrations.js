// No-op, so we get syntax highlighting with the SQL Tagged Template Literals VSCode extension,
	...
	function initial_schema({ db }) {
		...
		const home_page_doc = {
			...
			nodes: {
				...
				footer_1: {
					id: 'footer_1',
					type: 'footer',
					copyright: { text: 'Copyright © Editable Website', annotations: [] },
					footer_link_columns: [
						'footer_column_1',
						'footer_column_2',
						'footer_column_3',
		...
		db.exec(sql`
			CREATE TABLE documents (
				-- persistent id (usually a nanoid)
				document_id TEXT NOT NULL PRIMARY KEY,
				-- entry type (e.g. project, image, page)
				type TEXT NOT NULL,
				-- the data of your document (serialized_doc in json format)
				data TEXT
			);
		`);
		...
		db.prepare('INSERT INTO documents (document_id, type, data) VALUES(?, ?, ?)').run(
			'nav_1',
			'page',
			JSON.stringify(nav_doc)
		);

		db.prepare('INSERT INTO documents (document_id, type, data) VALUES(?, ?, ?)').run(
			'page_1',
			'page',
			JSON.stringify(home_page_doc)
		);
	}
];
