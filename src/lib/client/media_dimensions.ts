 ...
 */
function get_image_dimensions(blob: Blob): Promise<{ width: number; height: number }> {
 ...
 */
async function get_svg_dimensions(blob: Blob): Promise<{ width: number; height: number }> {
	try {
 ...
 */
export function get_video_dimensions(blob: Blob): Promise<{ width: number; height: number }> {
	return new Promise((resolve, reject) => {
		const video = document.createElement('video');
		video.preload = 'metadata';
		const object_url = URL.createObjectURL(blob);
		let poll_id: ReturnType<typeof setInterval>;
 ...
 */
export function get_media_dimensions(file: File): Promise<{ width: number; height: number }> {
