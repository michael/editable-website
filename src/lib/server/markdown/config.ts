...
/**
 * Validate and normalize markdown source mappings. Throws when the
 * configuration is invalid.
 */
export function validate_markdown_sources(sources: unknown): MarkdownSource[] {
	if (!Array.isArray(sources)) {
		throw new Error('MARKDOWN_SOURCES must be an array.');
	}

	const normalized: MarkdownSource[] = [];
	const seen_pathnames = new Set<string>();
...
function validate_pathname(pathname: unknown, label: string): string {
