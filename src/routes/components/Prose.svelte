<script>
	...
	import {
		TW_PAGE_PADDING_X,
		TW_BLOCK_PADDING_Y,
		TW_MOBILE_LEFT_INSET,
		TW_LIMITER
	} from '../tailwind_theme.js';
...
{#snippet content()}
	<NodeArrayProperty
		class="flex flex-col gap-5 md:gap-8 {heading_spacing}"
		path={[...path, 'content']}
	/>
{/snippet}
...
<Node
	class="ew-prose lg:text-lg layout-{layout} bg-(--background) text-(--foreground) {colorset_class}"
	{path}
>
