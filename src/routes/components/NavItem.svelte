...
{#snippet layout_2()}
	<svelte:element
		...
		href={render_as_link ? node.href : undefined}
		target={render_as_link ? node.target : undefined}
		class="flex items-center justify-center w-full h-full bg-(--accent) text-(--accent-foreground) uppercase font-medium rounded-(--button-border-radius) [corner-shape:squircle]"
