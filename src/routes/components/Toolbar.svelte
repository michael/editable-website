<script>
	...
	const TW_TOOLBAR_BTN =
		'flex items-center justify-center size-9 rounded-full text-(--foreground) bg-white cursor-pointer pointer-events-auto shadow-md transition-colors duration-150';
	const TW_TOOLBAR_BTN_DISABLED =
		'text-[oklch(from_var(--foreground)_0.8_0_0)] !cursor-not-allowed';
	const TW_TOOLBAR_BTN_HOVER =
		'hover:bg-[oklch(from_var(--svedit-brand)_0.95_0.02_h)] hover:text-(--svedit-editing-stroke)';
...
<div class="fixed {TW_TOOLBAR_POSITION} {TW_TOOLBAR_LEFT} pointer-events-none z-50 select-none">
	<div class="-mb-3 -ml-3 overflow-x-auto pb-3 pl-3">
		<div class="ml-auto flex w-max items-center gap-1.5 sm:gap-3">
			{#if !editable}
				<!-- Read mode: Edit button -->
				{#if !app_commands.edit_document.disabled}
					<button
						class="pointer-events-auto cursor-pointer rounded-full bg-(--svedit-brand) px-4 py-2 text-sm font-semibold text-white shadow-md transition-colors duration-150 hover:brightness-90"
			...
			{:else}
				...
				<div class="flex items-center gap-1">
					...
					<button
						class="{TW_TOOLBAR_BTN} {session.commands.cycle_node_type_next?.disabled
							? TW_TOOLBAR_BTN_DISABLED
							: TW_TOOLBAR_BTN_HOVER}"
					...
					<button
						class="{TW_TOOLBAR_BTN} {session.commands.cycle_layout_next?.disabled
							? TW_TOOLBAR_BTN_DISABLED
							: TW_TOOLBAR_BTN_HOVER}"
