<script>
	...
	let edit_image_command = $derived(svedit.session.commands?.edit_image);
	...
	function close() {
		if (edit_image_command) {
			edit_image_command.show_prompt = false;
		}
		svedit.focus_canvas();
	}
	...
	$effect(() => {
		if (edit_image_command?.show_prompt && dialog_ref) {
...
<dialog
	...
	class="edit-image-dialog absolute z-40 mt-1 m-0 bg-(--background) text-(--foreground) p-0 shadow-xl overflow-visible max-h-90 border border-[color-mix(in_oklch,var(--foreground)_18%,transparent)]"
...
>
	<div class="flex flex-col">
		...
		<textarea
			...
			class="w-72 min-w-0 border-0 bg-(--background) px-3 py-2 text-sm text-(--foreground) resize-none outline-none ring-0 focus:ring-0 focus-visible:ring-0 focus:outline-none focus-visible:outline-1 focus-visible:outline-(--svedit-editing-stroke) focus-visible:outline-offset-1"
