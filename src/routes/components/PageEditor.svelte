<script>
	...
	class SaveCommand extends Command {
		...
		async execute() {
			...
			] = await Promise.all([import('$lib/api.remote.js'), import('$lib/client/asset_upload.js')]);
	...
	let session = $derived(create_session(data.doc || demo_doc));
</script>

<svelte:window onkeydown={key_mapper.handle_keydown.bind(key_mapper)} />

<svelte:head>
	<title>{data.title || 'Editable Website'}</title>
</svelte:head>
